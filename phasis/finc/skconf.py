...
def struttura (c):
    ...
    c.setFont('Helvetica-Bold',14)
    c.drawString(320,130,"TOTALE Dare ")
...
WHERE anag.COD = CPART AND T_CPART="F" GROUP BY COD'''
    

def querycorpo ():
     return '''
SELECT CPART, substr(("("||NUM_MOV||")     Cod. cliente " ||CPART||"    "||DESCRIZ||"  ("||D_CONTO||") "),0,100) as DESCRIZ, IMPORVAL, SEGNO, CPART, D_CONTO
FROM movcon WHERE ANNO="%s" and CPART="%s" AND T_CPART="F" ORDER BY NUM_MOV '''

def querycalce ():
     return '''SELECT IFNULL(ROUND(-sum((IMPORVAL)),6),0) as TOT_DOC
FROM movcon
WHERE ANNO = "%s" AND CPART="%s" AND T_CPART="F" AND SEGNO="D" '''
